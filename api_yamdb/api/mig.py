...
cur.executemany("INSERT INTO reviews_titles (id,name,year,category_id) VALUES (?,?,?,?);", to_db)
con.commit()
con.close()
...
con = sqlite3.connect("../../db.sqlite3") # change to 'sqlite:///your_filename.db'
cur = con.cursor()
cur.execute("CREATE TABLE genre_title (id,title_id,genre_id);") # use your column names here
...
con = sqlite3.connect("../../db.sqlite3") # change to 'sqlite:///your_filename.db'
cur = con.cursor()
cur.execute("CREATE TABLE category (id,name,slug);") # use your column names here
...
cur.executemany("INSERT INTO reviews_comments (id,review_id,text,author,pub_date) VALUES (?,?,?,?,?);", to_db)
con.commit()
con.close()